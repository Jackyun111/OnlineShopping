...
<head>
    ...
    <script>
        ...
        function modifyName(){
            //alert(1);
            $.post(
                 "${pageContext.request.contextPath}/backend/productType/modifyName",{"id":$('#proTypeNum').val(),"name":$('#proTypeName').val()},
                    function(data){
                        if(data.status==1){
                            layer.msg(data.message,{
                                time:1000,
        ...
        function delProductType(){
            //alert(1);
            $.post(
                "${pageContext.request.contextPath}/backend/productType/deleteById",{"id":$('#productTypeId').val()},function(data){
                        if(data.status==1){
                            layer.msg(data.message,{
                                time:1000,
                                skin:"successMsg"
        ...
        function modifyStatus(id,btn){
            //alert(1);
            $.post(
                "${pageContext.request.contextPath}/backend/productType/modifyStatus",{"id":id},
                    function(){
...
<div class="panel panel-default" id="userSet">
    ...
    <div class="panel-body">
        <input type="button" value="添加" class="btn btn-primary" id="doAddProTpye">
        <br>
        <br>
                ...
                <c:forEach items="${data.list}" var="productType">
                    <tr>
                        ...
                        <td class="text-center">
                            <input type="button" class="btn btn-primary btn-sm doProTypeModify" value="修改" onclick="showProductType(${productType.id})">
